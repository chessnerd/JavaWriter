...
public class GUI {
	 ...
	 */
	public GUI() {
		// Try to look like a native program
		try {
	...
	public void makeFrame() {
		...
		// To be used for making constructors
		// panel1.add(blank4);
		// panel1.add(cNum);
		// panel1.add(cNumField);

		// Line needed until constructor functionality is fully added
		cNumField.setText("0");
	...
	private class MakeFieldListener implements ActionListener {
			...
			if (e.getSource() == start) {
				...
				if (fTypeField.getText().equals("")
					...
					JOptionPane.showMessageDialog(null,
							...
							"Missing Information", JOptionPane.ERROR_MESSAGE);
				} else if (!staticRadio.isSelected() && !intRadio.isSelected()) {
					JOptionPane.showMessageDialog(null,
		...
		public void makeMakeField() {
			...
			publicPanel.setLayout(new BoxLayout(publicPanel,
					BoxLayout.PAGE_AXIS));
			JPanel staticPanel = new JPanel();
			staticPanel.setLayout(new BoxLayout(staticPanel,
					BoxLayout.PAGE_AXIS));
	...
	private class RadioListener implements ActionListener {
		/**
		 * Takes an action depending on what radio button was clicked
		 */
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == privateRadio) {
				w.getPubPri()[currentField] = "private ";
				getter.setEnabled(true);
				if (!w.isFinal(currentField)) {
					setter.setEnabled(true);
				}
				...
				if (w.isFinal(currentField)) {
					w.setFinal(false, currentField);
					if(!(w.getPubPri()[currentField] == "public ")) {
						setter.setEnabled(true);
					}
				} else {
