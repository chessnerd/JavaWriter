...
public class Writer {
    ...
    /**
     * A method that takes a multi-word field name
     * and splits it into separate words broken up
     * by spaces
     */
    public String splitFieldName(String s) {
        ...
        for (int i = 0;  i < s.length();  i++ ) {
            char c = s.charAt(i);
            //If any two letters in a row are uppercase, return the original String
            if (Character.isUpperCase(c) && Character.isUpperCase(s.charAt(i+1))) {
            	return s;
            }
    ...
    public boolean testWrite() {
        ...
        if (pathNotSet) {
            ...
            filePath = filePath.substring(0,length-1)+filePath.substring(length);
        } else {
        	filePath = path;
        }
        ...
        if (hasError) {
        	JOptionPane.showMessageDialog(null, 
    			"Path " + filePath2 + " is not a writable location", 
    ...
    public void write() {
        ...
        for (int i = 0; i < fieldNum; i++) {
        	if (fieldTypes[i].equals("boolean")) {
        		printer.println("    /** Whether the " + splitFieldName(className) + " " + 
        				splitFieldName(fieldNames[i]) + "*/");
        	} else {
        		printer.println("    /** The " + splitFieldName(fieldNames[i]) + " of the " + 
                    	splitFieldName(className) + " */");
        	}
            printer.println("    " + pubPri[i] + stat[i] + fin[i] + fieldTypes[i] + 
                " " + fieldNames[i] + ";");
            printer.println("    ");
        }
        ...
        for (int i = 0; i < fieldNum; i++) {
            if(fieldGet[i]) {
                printer.println("    /**");
                printer.println("     * Accessor method for the " + fieldNames[i] + " field");
                printer.println("     * ");
                printer.println("     * @return gives the " + splitFieldName(className) + "'s " + splitFieldName(fieldNames[i]));
                printer.println("     */");
                printer.println("    public " + stat[i] + fieldTypes[i] + " get" + capitalize(fieldNames[i]) + "(){");
            ...
            if(fieldSet[i]) {
                printer.println("    /**");
                printer.println("     * Mutator method for the " + fieldNames[i] + " field");
                printer.println("     * ");
                printer.println("     * @param p" + capitalize(fieldNames[i]) + " takes a new " + fieldTypes[i] + " for the " + splitFieldName(className) + "'s " + splitFieldName(fieldNames[i]));
                printer.println("     */");
                printer.println("    public " + stat[i] + "void set" + capitalize(fieldNames[i]) + "(" + fieldTypes[i] + " p" + capitalize(fieldNames[i]) + "){");
                printer.println("        " + fieldNames[i] + " = p" + capitalize(fieldNames[i]) + ";");
                printer.println("    }");
                printer.println("    ");
            }
        }
     ...
     * 
     * @param pFileName takes a new String for the file name
     */
     ...
     * 
     * @param pPath takes a new String for the path
     */
     ...
     * 
     * @param pName takes a new String for name
     */
     ...
     * 
     * @param pDate takes a new String for date
     */
     ...
     * 
     * @param pClassName takes a new String for className
     */
     ...
     * 
     * @param pConNum takes a new int for conNum
     */
     ...
     * 
     * @param b whether the field is private
     * @param i the field being set for
     */
     ...
     * 
     * @param b whether the field is static
     * @param i the field being set for
     */
     ...
     * 
     * @param b whether the field is final
     * @param i the field being set for
     */
